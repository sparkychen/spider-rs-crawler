...
# Bazel构建二进制（适配项目构建规则）
build_binary() {
  log_info "开始构建${BUILD_MODE}模式二进制（改用Cargo构建）..."
  export CC=gcc-11 CXX=g++-11
  local cargo_build_flags=()
  # 适配Debug/Release模式
  if [[ "$BUILD_MODE" == "release" ]]; then
    cargo_build_flags+=("--release")
    # Release模式产物路径
    local cargo_bin_path="target/release/spider-enterprise-crawler"
  else
    # Debug模式产物路径
    local cargo_bin_path="target/debug/spider-enterprise-crawler"
  fi
  
  # 执行Cargo构建（结合xvfb-run，避免构建时弹出Chrome窗口）
  xvfb-run --auto-servernum cargo build "${cargo_build_flags[@]}"
  
  # 检查产物是否生成
  if [[ ! -f "$cargo_bin_path" ]]; then
    log_error "Cargo构建失败，未找到二进制文件：$cargo_bin_path"
  fi
  
  # 复制二进制到dist目录（保持原脚本的产物路径）
  cp "$cargo_bin_path" dist/spider-enterprise-crawler
  # 添加执行权限
...
# 修正后的build_bundle函数（删除重复复制二进制的代码）
build_bundle() {
  log_info "开始构建企业级分发包（手动打包，替代Bazel）..."
  
  # 1. 确保dist目录存在
  mkdir -p dist/config dist/deploy dist/scripts
  
  # 2. 【删除重复复制二进制的代码】二进制已由build_binary复制到dist，无需重复操作
  
  # 3. 复制核心配置
  cp config/crawler.yaml dist/config/ || log_warn "爬虫配置文件 config/crawler.yaml 复制失败"
  ...
  # 6. 复制部署文件（Docker/K8s）
  if [[ -d deploy ]]; then
    cp -r deploy/* dist/deploy/ || log_warn "部署文件 deploy/ 复制失败"
  fi
  
  # 7. 复制构建脚本（可选，便于分发后重新构建）
  if [[ -f scripts/build.sh ]]; then
    cp scripts/build.sh dist/scripts/ || log_warn "构建脚本 scripts/build.sh 复制失败"
    chmod +x dist/scripts/build.sh
  fi
  
  # 8. 生成分发包说明文件
  cat > dist/README.md << 'EOF'
# spider-enterprise-crawler 企业级分发包
## 目录结构
- ./spider-enterprise-crawler：爬虫二进制文件（Release模式优化）
- ./config/crawler.yaml：爬虫核心配置
- ./run_crawler.sh：快速启动脚本
- ./deploy/：Docker/K8s部署配置
- ./scripts/build.sh：构建脚本
## 使用方式
1. 复制 .env.example 为 .env，配置敏感信息（登录/MinIO/Redis）
2. 执行 ./run_crawler.sh 启动爬虫
3. 如需重新构建，执行 ./scripts/build.sh --release
EOF
